...
Fixtures.prototype.load = function () {
  ...
  return readdir(this._dir)
    .then(files => {
      ...
      return Promise.all(promises).tap(() => log(this._mute, '[done] *load all'));
    })
  ...
  return readdir(this._dir).then(files => {
    const promises = files.map(file => {
      ...
      return this._db.collection(collectionName)
        ...
        .tapCatch(e => log(this._mute, '[error] unload', collectionName, e.message));
    });
    return Promise.all(promises).tap(() => log(this._mute, '[done] *unload all'));
  });
};
