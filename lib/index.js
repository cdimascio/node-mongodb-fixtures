      ...
      const promises = files.filter(this._filterFiles).map(file => {
        ...
        if (!isSupportedExt(ext)) {
          ...
          return readCollectionFromJsJson(path.join(this._dir, file))
            ...
            .then(bulkInsertDocuments(collectionName));
        }
      });
...
Fixtures.prototype.unload = function() {
  ...
  return readdir(this._dir).then(files => {
    const promises = files.filter(this._filterFiles).map(file => {
      ...
      const dropIndexes = this._db
        .collection(collectionName)
        .dropIndexes()
        .tap(() => log(this._mute, '[info ] dropped indexes', collectionName))
        .catch(e => {
          if (e.code !== 26) throw e;
        });

      log(this._mute, '[start] unload', collectionName);
      const dropDocs = this._db
        .collection(collectionName)
        .deleteMany()
      ...
      return Promise.all([dropIndexes, dropDocs]);
    });
